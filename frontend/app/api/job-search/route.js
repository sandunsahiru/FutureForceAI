...
export async function POST(request) {
  try {
    ...
    const apiUrl = `${backendUrl}/api/job-search`;
    ...
    const response = await fetch(apiUrl, {
      ...
      // Add these options to handle potential network issues
      cache: 'no-store',
      next: { revalidate: 0 },
    });
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { detail: errorData.detail || `Failed to search jobs (Status: ${response.status})` },
    ...
    console.error('Error in job search API route:', error);
